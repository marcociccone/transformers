...
class BoomConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`BoomModel`]. It is used to instantiate a
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 128256):
            ...
            `inputs_ids` passed when calling [`BoomModel`]
        hidden_size (`int`, *optional*, defaults to 5120):
            Dimension of the hidden representations.
        intermediate_size (`int`, *optional*, defaults to 17408):
    ...
    ```python
    >>> from transformers import BoomModel, BoomConfig

    >>> # Initializing a Boom style configuration
    >>> configuration = BoomConfig()

    >>> # Initializing a model from the Boom style configuration
    >>> model = BoomModel(configuration)
...
class BoomRMSNorm(LlamaRMSNorm):
    pass


class BoomAttention(LlamaAttention):
    def __init__(self, config: BoomConfig, layer_idx: int):
        super().__init__(config, layer_idx)

        self.use_rope = config.no_rope_layers[layer_idx]
        self.q_norm = BoomRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
        self.k_norm = BoomRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # thus post q_norm does not need reshape
        self.sliding_window = (
...
class BoomDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config: BoomConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        self.attention_type = config.layer_types[layer_idx]


class BoomPreTrainedModel(LlamaPreTrainedModel):
    pass


class BoomModel(Qwen2Model):
    pass


class BoomForCausalLM(LlamaForCausalLM):
    pass


class BoomForSequenceClassification(LlamaForSequenceClassification):
    pass


class BoomForTokenClassification(LlamaForTokenClassification):
    pass


class BoomForQuestionAnswering(LlamaForQuestionAnswering):
    pass
...
__all__ = [
    ...
    "BoomModel",
    "BoomForCausalLM",
    "BoomForSequenceClassification",
    "BoomForTokenClassification",
    "BoomForQuestionAnswering",
]
